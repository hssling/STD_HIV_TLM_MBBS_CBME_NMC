#!/usr/bin/env python3
"""
STD Presentation Generator
Creates PowerPoint presentation for STD education
...
def create_std_presentation():
    ...
    p = tf.add_paragraph()
    p.text = '• Bacterial: Gonorrhea, Syphilis, Chlamydia'
    p.level = 2

    p = tf.add_paragraph()
    p.text = '• Viral: HIV, HSV, HPV, Hepatitis B'
    p.level = 2

    p = tf.add_paragraph()
    p.text = '• Parasitic: Trichomoniasis, Pubic lice'
    p.level = 2

    p = tf.add_paragraph()
    p.text = '• Fungal: Candidiasis'
    p.level = 2
    ...
    tf = body_shape.text_frame
    tf.text = 'WHO Statistics (2022):'

    p = tf.add_paragraph()
    p.text = '• 1 million new STD cases daily'
    p.level = 1

    p = tf.add_paragraph()
    p.text = '• 376 million new cases annually'
    p.level = 1

    p = tf.add_paragraph()
    p.text = '• Chlamydia: 129 million'
    p.level = 1

    p = tf.add_paragraph()
    p.text = '• Gonorrhea: 82 million'
    p.level = 1

    p = tf.add_paragraph()
    p.text = '• Syphilis: 7.1 million'
    p.level = 1

    p = tf.add_paragraph()
    p.text = '• Trichomoniasis: 156 million'
    p.level = 1
    ...
    prs.save('STD_One_Hour_Class.pptx')
    print("STD presentation created successfully!")

if __name__ == "__main__":
    create_std_presentation()
